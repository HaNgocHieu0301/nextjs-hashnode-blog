...
export const Search = () => {
	const { publication, language } = useAppContext();
	...
	const search = async (query: string) => {
		...
		timerRef.current = setTimeout(async () => {
			...
			const posts = data.searchPostsOfPublication.edges
				.map((edge) => edge.node)
				.filter((post) => {
					if (language === 'en') {
						// For English, include posts with '-en' at the end of the slug
						return post.slug.endsWith('-en');
					} else {
						// For Vietnamese (or any non-English), exclude posts with '-en' at the end
						return !post.slug.endsWith('-en');
					}
				});
	...
	const searchResultsList = searchResults.map((post) => {
		const postURL = `/${language}/${post.slug}`;
		return (
			<Link
				key={post.id}
				href={postURL}
					...
					{searchResults.length > 0 && !isSearching && (
						<div className="top-100 absolute left-0 z-10 mt-1 flex w-full flex-col items-stretch overflow-hidden rounded-lg border bg-white p-1 text-left text-slate-900 shadow-2xl dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-50">
							<h3 className="px-4 py-2 font-medium text-slate-500 dark:text-neutral-400">
								{t('found')} {searchResults.length} {t('results')}
							</h3>
